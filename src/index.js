        ...
        Object.entries(data.species).forEach(([category, speciesList]) => {
            ...
            const categoryHtml = `
                <div class="flex justify-between items-center bg-white text-customBlue p-4 rounded-t-md">
                    ...
                    <button class="toggle-category focus:outline-none" data-target="${category.replace(/\s+/g, '-')}Cards">
            ...
            const cardsHtml = speciesList.map(species => {
                const truncatedDescription = species.description.length > 100 ? species.description.substring(0, 100) + '...' : species.description;
                    return `
                        <div class="card max-w-md bg-white rounded-lg border border-gray-200 shadow-md m-8 p-4 cursor-pointer" role="button" onclick="openModal('${species.common_name}', '${species.image}', '${species.description}')">
                            ...
                            <div class="flex -mx-4">
                                <img src="${species.image}" alt="${species.common_name}" class="w-1/2 h-auto rounded-md px-4">
...
// Adjust loadSpeciesData to call displaySpecies with the entire data object
